...
namespace CityApp.Client.Components.CardNewRutaRecoleccionComponents
{
    public partial class CardNewRutaRecoleccion
    {
        ...
        private void SelectDia(ChangeEventArgs args)
        {
            try
            {
                ...
                if (idDiaRuta != 0)
                {
                    ...
                    switch (idDiaRuta)
                    {
                        case 0: diasSeleccionados = new string[] { "Domingo" }; break;
                        case 1: diasSeleccionados = new string[] { "Lunes" }; break;
                        case 2: diasSeleccionados = new string[] { "Martes" }; break;
                        case 3: diasSeleccionados = new string[] { "Miércoles" }; break;
                        case 4: diasSeleccionados = new string[] { "Jueves" }; break;
                        case 5: diasSeleccionados = new string[] { "Viernes" }; break;
                        case 6: diasSeleccionados = new string[] { "Sábado" }; break;
                    }
        ...
        private async void GuardarRutaRecoleccion()
        {
            if (!banderaBoton)
            {
                banderaBoton = true;
                        ...
                        if (RutaRecoleccion.NombreRuta != "NA" && RutaRecoleccion.NombreRuta != "")
                        {
                            if (RutaRecoleccion.Horario != "NA" && RutaRecoleccion.Horario != "")
                            {
                                if(RutaRecoleccion.IdCuenta != 0)
                                {
                                    ...
                                    InsertRutaRecoleccionLogic insertRutaRecoleccionLogic = new InsertRutaRecoleccionLogic(Cliente);
                                    response = await insertRutaRecoleccionLogic.Insert(Sesion.TokenAcceso, RutaRecoleccion);
                                    if (response.Status.Exito == 1)
                                    {
                                        carrusel1 = "no_view";
                                        carrusel2 = "";
