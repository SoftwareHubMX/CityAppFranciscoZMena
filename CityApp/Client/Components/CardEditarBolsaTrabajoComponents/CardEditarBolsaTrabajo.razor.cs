...
namespace CityApp.Client.Components.CardEditarBolsaTrabajoComponents
{
    public partial class CardEditarBolsaTrabajo
    {
        [Parameter] public int IdBolsaTrabajo { get; set; } = 0;
        ...
        private async void ConsultarBolsaTrabajo()
        {
            ...
            SelectBolsaTrabajoLogic selectBolsaTrabajoLogic = new SelectBolsaTrabajoLogic(Cliente);
            response = await selectBolsaTrabajoLogic.Select(Sesion.TokenAcceso, IdBolsaTrabajo);
            if (response.Status.Exito == 1)
            {
                BolsaTrabajo = response.Data;
                puesto = BolsaTrabajo.Puesto;
                empresa = BolsaTrabajo.Empresa;
                escolaridad = BolsaTrabajo.Escolaridad;
                especialidad = BolsaTrabajo.Especialidad;
                edad = BolsaTrabajo.Edad;
                sexo = BolsaTrabajo.Sexo;
                experiencia = BolsaTrabajo.Experiencia;
                conocimientos = BolsaTrabajo.Conocimiento;
                habilidades = BolsaTrabajo.Habilidades;
                funciones = BolsaTrabajo.Funciones;
                jornada = BolsaTrabajo.Jornada;
                sueldo = BolsaTrabajo.Sueldo;
                prestaciones = BolsaTrabajo.Prestaciones;
                otrasPrestaciones = BolsaTrabajo.OtrasPrestaciones;
                requisitos = BolsaTrabajo.Requisitos;
                entrevista = BolsaTrabajo.Entrevisata;
                contacto = BolsaTrabajo.Contacto;
                calle = BolsaTrabajo.Calle;
                numero = BolsaTrabajo.Numero;
                codigoPostal = BolsaTrabajo.CodigoPostal;
                colonia = BolsaTrabajo.Colonia;
                localidad = BolsaTrabajo.Localidad;
                telefono1 = BolsaTrabajo.Telefono1;
                telefono2 = BolsaTrabajo.Telefono2;
                correo = BolsaTrabajo.Correo;
                actividadPrincipal = BolsaTrabajo.ActividadPrincipal;
                rfc = BolsaTrabajo.Rfc;
                plaza = BolsaTrabajo.Plaza;
            }
            else
            {
                alerta = response.Status.Mensaje;
        ...
        private void TxtSexo(ChangeEventArgs args)
        {
            ...
            if (sexo != "")
            {
                errorSexo = "";
                StateHasChanged();
                if (Validaciones.ValidarCaracteres(sexo))
                {
                    errorSexo = "";
                    BolsaTrabajo.Sexo = sexo;
                }
                else
                {
                    errorSexo = "NoCaracteresEspeciales";
                    sexo = "";
                    BolsaTrabajo.Sexo = "NA";
                }
            }
            StateHasChanged();
        }
        ...
        private async void ActualizarBolsaTrabajo()
        {
            if (!banderaBoton)
            {
                banderaBoton = true;
                ...
                if(BolsaTrabajo.IdGiro != 0)
                {
                    ...
                    UpdateBolsaTrabajoLogic updateDependenciaLogic = new UpdateBolsaTrabajoLogic(Cliente);
                    response = await updateDependenciaLogic.Update(Sesion.TokenAcceso, BolsaTrabajo);
                    if (response.Status.Exito == 1)
                    {
