...
namespace CityApp.Client.Components.CardNuevoTramiteComponents
{
    public partial class CardNuevoTramite
    {
                    ...
                    if(Tramite.Descripcion != "NA" && Tramite.Descripcion != "")
                    {
                        if (Tramite.IdDependencia != 0)
                        {
                            if (Tramite.IdTipoTramite != 0)
                            {
                                ...
                                InsertTramiteLogic insertTramiteLogic = new InsertTramiteLogic(Cliente);
                                response = await insertTramiteLogic.Insert(Sesion.TokenAcceso, Tramite);
                                if (response.Status.Exito == 1)
                                {
