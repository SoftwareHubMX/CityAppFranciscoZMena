...
{
    public class DescargarArchivoNoticia
    {
        private HttpClient Cliente;
        public DescargarArchivoNoticia(HttpClient cliente)
        {
            Cliente = cliente;
        }

        public async Task<Response<byte[]>> DescargarArchivoNoticiaAsync(string imagen, int idNotocia)
        {
            Response<byte[]> response = new Response<byte[]>();

            string url = "ArchivoNoticia/DescargarArchivoNoticia";
