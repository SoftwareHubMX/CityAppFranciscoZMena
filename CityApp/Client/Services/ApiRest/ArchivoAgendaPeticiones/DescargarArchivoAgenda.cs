...
{
    public class DescargarArchivoAgenda
    {
        private HttpClient Cliente;
        public DescargarArchivoAgenda(HttpClient cliente)
        {
            Cliente = cliente;
        }

        public async Task<Response<byte[]>> DescargarArchivoAgendaAsync(string imagen, int idNotocia)
        {
            Response<byte[]> response = new Response<byte[]>();

            string url = "ArchivoAgenda/DescargarArchivoAgenda";
